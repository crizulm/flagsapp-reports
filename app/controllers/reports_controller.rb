class ReportsController < ApplicationController
  ...
  def create
    token = params[:token]
    return render json: { error: 'The flag token is NULL' }, status: :bad_request if token.nil?
    ...
    if @report.save
      ...
      return render json: { error: 'Error in flag creation' }, status: :bad_request
    end
  end
  ...
  def show
    ...
    return render json: { error: 'The report for flag ' + token + ' not found' }, status: :bad_request if report.nil?
  ...
  def update_result
    ...
    return render json: { error: 'The flag evaluate result is NULL' }, status: :bad_request if result.nil?
    ...
    if report.save
      ...
      return render json: { error: 'Error in flag update' }, status: :bad_request
    end
  end
  ...
  def update_time
    token = params[:token]
    time = params[:time]
    return render json: { error: 'The flag evaluate time is NULL' }, status: :bad_request if time.nil?
